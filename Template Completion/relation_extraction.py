...
def main(args):
    ...
    for data in datas:
        ...
        try:
            ...
            extract_data = data['extract_data']
            extract_workfolw.entity_requirements = method_2_needed_para[extract_workfolw.method] if extract_workfolw.method else None
            extract_data = {k:v for k,v in extract_data.items() if k in extract_workfolw.entity_requirements}
            extract_workfolw.extract_data = process_extract_data(extract_data)
            extract_workfolw.part_2()
            extract_workfolw.part_3()
            extract_workfolw.part_4()
            ...
            logging.info(f'Unexpected error processing data for doi {doi}: {e}')
            write_json(args.output_path, cur_data)
            continue
